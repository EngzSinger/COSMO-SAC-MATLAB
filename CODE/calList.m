...
SystemIDAC=ReadIDACList('CombList.txt');
...
%Err=sum(Errg);
	pathResult=['/home/zxwu/WORK_SPACE/COSMO_SAC/RESULT/Result_comb.txt'];
...
%	fprintf(fid,'%s\t%.4f\n\n','AARD=',Err/Nsys);
	fprintf(fid,'%s\t%s\t%s\t%s\t%s\t%s\t\n','No','Cation','Anion','Solute','T','Vcal');
	for i=1:Nsys
		fprintf(fid,'%d\t%s\t%s\t%s\t%8.2f\t%.4f\t\n',i,SystemIDAC{1,2}{i},SystemIDAC{1,3}{i},SystemIDAC{1,1}{i},SystemIDAC{1,4}(i),gam(i));
	end
	fclose(fid);
